...
(function($) {
	...
	var flowApp = {
		...
		getUsgsData: function(river) {
			...
			// display the usgs graph image
			// flowApp.displayGraph();
			// display canvas graph
				...
				.success(function(data) {
					...
					if (data.value.timeSeries.length === 0) {
						...
						$.each(data.value.timeSeries, function(i, item) {
							...
							flowApp.displayChart(item.values[0].value);
						}); // END $.each
		...
		displayChart: function(graphData) {
			var chart = document.getElementById("flowChart");
			var dataSet = graphData;
			// reduce values by date
			var lessData = dataSet.map(function(item) {
				//console.log(item);
			});

			var chartDatapoints = dataSet.map(function(item) {
				return parseInt(item.value, 10);
			});
			// var chartDatapoints = _.map(dataPoints, "value"); // cfs
			...
			var flowChart = new Chart(chart, {
				type: 'bar',
				data: {
					labels: chartLabels, // ["date","date"...],
					datasets: [{
						label: 'CFS',
						data: chartDatapoints // [12, 19, 3, 5, 2, 3]
					}]
				}
			});
		},
